...
const storage = diskStorage({
  destination: (req, file, cb) => {
    ...
    const uploadPath = path.join(process.cwd(), 'uploads', 'tasks', taskId);
...
export class TaskFilesController {
  ...
  async uploadFile(
    ...
    const relativePath = path.join('uploads', 'tasks', taskId, file.filename);

    const taskFile = await this.taskFilesService.create({
  ...
  async findByTaskId(@Param('taskId') taskId: string) {
    return await this.taskFilesService.findByTaskId(parseInt(taskId));
  }

  @Get(':id/download')
  async downloadFile(@Param('id') id: string, @Res() res: Response) {
    const file = await this.taskFilesService.findOne(parseInt(id));
    const filePath = await this.taskFilesService.getFilePath(parseInt(id));
  ...
  async remove(@Param('id') id: string) {
    await this.taskFilesService.remove(parseInt(id));
