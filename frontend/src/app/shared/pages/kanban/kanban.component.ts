...
export class KanbanComponent implements OnInit {
  ...
  get roleBasePath(): string {
    ...
    return roleMap[this.currentUser?.rol || ''] || 'admin';
  }

  get canCreateTasks(): boolean {
    return this.currentUser?.rol !== 'Empleado' && this.currentUser?.rol !== 'Cliente';
  }
  ...
  ngOnInit(): void {
    this.route.paramMap.subscribe((params) => {
      ...
      this.tasksService.findByProjectId(this.projectId).subscribe((tasks) => {
        // Lógica condicional: Empleados solo ven sus tareas, otros roles ven todas
        const isEmployee = this.currentUser?.rol === 'Empleado';
        const filteredTasks = isEmployee
          ? tasks.filter((t) => t.creadoPorId === this.currentUserId)
          : tasks;

        for (const est of Object.values(EstadoTarea))
          this.tasksByEstado[est] = filteredTasks.filter((t) => t.estado === est);
